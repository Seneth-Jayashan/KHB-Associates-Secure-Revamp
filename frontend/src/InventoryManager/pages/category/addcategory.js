  ...
  const getBrands = async () => {
    try {
      ...
      if (!response.data || response.data.length === 0) {
        toast.error("No brands found, A brand must be added before adding categories");
      ...
      if (response.status === 200) {
        try {
          ...
          if (catCount.status === 200) {
            ...
            toast.error("Failed to update category count.");
          }
        } catch (error) {
          toast.error(error.response?.data?.error || "Error updating category count.");
        }
      } else {
        toast.error(response.data.error || "An error occurred while adding category.");
      }
    } catch (error) {
