...
export default function Cart() {
    ...
    const fetchCartAndProducts = async () => {
      try {
        ...
        const updatedCartItems = cartData.items.filter(
          (item) => validProductIds.includes(item.product_id)
        );
        ...
        console.error(
          'Error fetching cart or products:',
          err
        );
  ...
  useEffect(() => {
    ...
    axios.put(
      'http://localhost:3001/api/cart/updatetotalprice',
      {
  ...
  return (
    <div>
      <div className="p-10 bg-gray-100 min-h-screen flex flex-col">
        ...
        <Link
          to="/checkout"
          className="mt-6 px-6 py-3 bg-custom-gradient text-white rounded-lg text-center block"
        >
          Proceed to Checkout
        </Link>
        ...
        <Link
          to="/shop"
          className="mt-4 px-6 py-3 bg-gray-500 text-white rounded-lg text-center block"
        >
          Back to Shop
        </Link>
