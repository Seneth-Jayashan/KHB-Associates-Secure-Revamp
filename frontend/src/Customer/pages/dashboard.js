...
export default function Dashboard() {
  ...
  // Fetch ticket statistics after user data is available
  useEffect(() => {
    if (userData) {
      axios
        .get(
          `http://localhost:3001/api/tickets/tickets/${userData.user_id}`,
          {
        ...
        .catch((error) => {
          console.error(
            "There was an error fetching the tickets!",
            error
          );
        });
    }
  ...
  // Get authenticated user from the backend
  ...
  // Fetch authenticated user when dashboard loads
  useEffect(() => {
    fetchUserData();
  }, []);

  // Fetch order summary after user data is available
  useEffect(() => {
    if (userData?.user_id) {
