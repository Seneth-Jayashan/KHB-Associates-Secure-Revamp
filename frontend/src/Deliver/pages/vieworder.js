...
export default function ViewOrder() {
  ...
  useEffect(() => {
    const fetchOrderDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/api/orders/${orderId}`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
        ...
        const user = await axios.get(`http://localhost:3001/api/customers/customer?id=${order.user_id}`);
    ...
    <div className="p-10 bg-gray-100 min-h-screen flex flex-col">
      ...
      <div className="bg-white p-6 rounded-xl shadow-md">
        ...
        <p className="text-lg text-gray-600 mb-2">
          Customer Name: {customerData ? `${customerData.firstName} ${customerData.lastName}` : 'Loading...'}
        </p>
