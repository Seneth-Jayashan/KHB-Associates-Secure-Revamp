...
export default function Deliveries() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/api/delivers/alldeliveries?id=${deliverId}`);
  ...
  return (
    <div className="p-6">
      <h2 className="text-3xl font-bold text-gray-800 mb-6">Delivered Orders</h2>
      {orders.length === 0 ? (
